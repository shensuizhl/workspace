...
function basepay_page(	){
	$("#pagination").on("click", "li", function() {
		...
		if (aText == "上一页") {
			...
			basepay_getList(currentPage - 1, size);
			checkA();
			...
			$(".userListTable tbody").empty();
			basepay_getList($(this).text(), size);
			checkA();
		}
	})
	//分页栏右侧下拉列表的实现
...
//与后台获取数据，加载到页面上
function basepay_getList(pageNum, size) {
	currentPage = pageNum;
	//alert(termination);
				//alert(currentPage);
						...
						for (var i = 0; i < length; i++) {
							...
							$(".userListTable tbody")
								.append('<tr id="mytr">'
												...
												+ grade
												+ '</td>'
+ '<td><button type="button" class="btn btn-success btn-xs" onclick="basepay_updatePrepare();basepay_findOne('+basepayId+')" data-toggle="modal" data-target="#editModal">修改</button><button type="button" class="btn btn-danger btn-xs" onclick="deleteOne('+basepayId+')">删除</button></td>'
										+ '</tr>')
...
function basepay_findOne(id){
  ...
  //alert(result.children("td").get(1).innerHTML);  
	...
	$("#payment").val(cells[3].innerHTML);
	$("#grade").val(cells[4].innerHTML);
...
//修改结束
var hasId=true;
function basepay_updatePrepare(){
	if(hasId){
		$("#specialDiv").attr("style","display: none;");
		hasId=false;
	}
	$("#saveBtn").attr("onclick","");
	$("#saveBtn").attr("onclick","basepay_updateOne()");
}
