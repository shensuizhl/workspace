...
function age_page(	){
	$("#pagination").on("click", "li", function() {
		...
		if (aText == "上一页") {
			...
			age_getList(currentPage - 1, size);
			checkA();
...
//与后台获取数据，加载到页面上
function age_getList(pageNum, size) {
	currentPage = pageNum;
	//alert(termination);
				//alert(currentPage);
				...
				success : function(data) {
					...
					if (data.code == "200") {
						...
						for (var i = 0; i < length; i++) {
							var agId = data.data.dataList[i].agId;
							...
							var genderKey = data.data.dataList[i].genderKey;
							var noCh = data.data.dataList[i].noCh;
							var ageOfEmployee = data.data.dataList[i].ageOfEmployee;
							var maritalStatusKey = data.data.dataList[i].maritalStatusKey;
							...
							$(".userListTable tbody")
								.append('<tr id="mytr">'
												...
												+ maritalStatusKey
												+ '</td>'
+ '<td><button type="button" class="btn btn-success btn-xs" onclick="age_updatePrepare();age_findOne('+agId+')" data-toggle="modal" data-target="#editModal">修改</button><button type="button" class="btn btn-danger btn-xs" onclick="deleteOne('+agId+')">删除</button></td>'
										+ '</tr>')
...
function age_findOne(id){
  ...
  //alert(result.children("td").get(1).innerHTML);  
	...
	$("#genderKey").val(cells[3].innerHTML);
	$("#noCh").val(cells[4].innerHTML);
	$("#ageOfEmployee").val(cells[5].innerHTML);
	$("#maritalStatusKey").val(cells[6].innerHTML);
	var inp=document.getElementById("editModal"); 
	inp.setAttribute("agId",id);//
...
function age_updateOne(){
	...
	row.agId=inp2.attributes["agId"].nodeValue;
	...
	row.genderKey=$("#genderKey").val();
	row.noCh=$("#noCh").val();
	row.ageOfEmployee=$("#ageOfEmployee").val();
	row.maritalStatusKey=$("#maritalStatusKey").val();
	$.ajax({
		url : ip+'/demo/table/modify/ageGender',
...
//修改结束
var hasId=true;
function age_updatePrepare(){
	if(hasId){
		$("#specialDiv").attr("style","display: none;");
		hasId=false;
	}
	$("#saveBtn").attr("onclick","");
	$("#saveBtn").attr("onclick","age_updateOne()");
}
