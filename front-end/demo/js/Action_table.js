...
function action_page(	){
	$("#pagination").on("click", "li", function() {
		...
		if (aText == "上一页") {
			...
			action_getList(currentPage - 1, size);
			checkA();
...
//与后台获取数据，加载到页面上
function action_getList(pageNum, size) {
	currentPage = pageNum;
	//alert(termination);
				//alert(currentPage);
				...
				success : function(data) {
					...
					if (data.code == "200") {
						...
						for (var i = 0; i < length; i++) {
							var actionId = data.data.dataList[i].actionId;
							...
							var actionType = data.data.dataList[i].actionType;
							var reasonForAction = data.data.dataList[i].reasonForAction;
							var last = data.data.dataList[i].last;
							var entry = data.data.dataList[i].entry;
							var startDate1 = data.data.dataList[i].startDate1;
							var endDate1 = data.data.dataList[i].endDate1;
							var position = data.data.dataList[i].position;
							id='mytr'+actionId
							...
							$(".userListTable tbody")
								.append('<tr id="mytr">'
												...
												+ position
												+ '</td>'
+ '<td><button type="button" class="btn btn-success btn-xs" onclick="action_updatePrepare();action_findOne('+actionId+')" data-toggle="modal" data-target="#editModal">修改</button><button type="button" class="btn btn-danger btn-xs" onclick="deleteOne('+actionId+')">删除</button></td>'
										+ '</tr>')
...
function action_findOne(id){
  ...
  //alert(result.children("td").get(1).innerHTML);  
	...
	$("#actionType").val(cells[4].innerHTML);
	$("#reasonForAction").val(cells[5].innerHTML);
	$("#last").val(cells[6].innerHTML);
	$("#entry").val(cells[7].innerHTML);
	$("#startDate1").val(cells[8].innerHTML);
	$("#endDate1").val(cells[9].innerHTML);
	$("#position").val(cells[10].innerHTML);
	var inp=document.getElementById("editModal"); 
	inp.setAttribute("actionId",id);//
...
function action_updateOne(){
	...
	row.actionId=inp2.attributes["actionId"].nodeValue;
	...
	row.actionType=$("#actionType").val();
	row.reasonForAction=$("#reasonForAction").val();
	row.last=$("#last").val();
	row.entry=$("#entry").val();
	row.reasonForAction=$("#startDate1").val();
	row.last=$("#endDate1").val();
	row.entry=$("#position").val();
	$.ajax({
		url : ip+'/demo/table/modify/actionPosition',
...
//修改结束
var hasId=true;
function action_updatePrepare(){
	if(hasId){
		$("#specialDiv").attr("style","display: none;");
		hasId=false;
	}
	$("#saveBtn").attr("onclick","");
	$("#saveBtn").attr("onclick","action_updateOne()");
}
