...
function address_page(	){
	$("#pagination").on("click", "li", function() {
		...
		if (aText == "上一页") {
			...
			address_getList(currentPage - 1, size);
			checkA();
			...
			$(".userListTable tbody").empty();
			address_getList($(this).text(), size);
			checkA();
		}
	})
	//分页栏右侧下拉列表的实现
...
//与后台获取数据，加载到页面上
function address_getList(pageNum, size) {
	currentPage = pageNum;
	//alert(termination);
				//alert(currentPage);
				...
				success : function(data) {
					...
					if (data.code == "200") {
						...
						for (var i = 0; i < length; i++) {
							var addressId = data.data.dataList[i].addressId;
							...
							var addressRecordType = data.data.dataList[i].addressRecordType;
							var streetAndHouseNumber = data.data.dataList[i].streetAndHouseNumber;
							...
							$(".userListTable tbody")
								.append('<tr id="mytr">'
												...
												+ '</td>'
+ '<td><button type="button" class="btn btn-success btn-xs" onclick="address_updatePrepare();address_findOne('+addressId+')" data-toggle="modal" data-target="#editModal">修改</button><button type="button" class="btn btn-danger btn-xs" onclick="deleteOne('+addressId+')">删除</button></td>'
										+ '</tr>')
...
function address_findOne(id){
  ...
  //alert(result.children("td").get(1).innerHTML);  
	...
	$("#addressRecordType").val(cells[3].innerHTML);
	$("#streetAndHouseNumber").val(cells[4].innerHTML);
	var inp=document.getElementById("editModal"); 
	inp.setAttribute("addressId",id);//
...
function address_updateOne(){
	...
	row.addressId=inp2.attributes["addressId"].nodeValue;
	...
	row.addressRecordType=$("#addressRecordType").val();
	row.streetAndHouseNumber=$("#streetAndHouseNumber").val();
	$.ajax({
		url : ip+'/demo/table/modify/address',
...
//修改结束
var hasId=true;
function address_updatePrepare(){
	if(hasId){
		$("#specialDiv").attr("style","display: none;");
		hasId=false;
	}
	$("#saveBtn").attr("onclick","");
	$("#saveBtn").attr("onclick","address_updateOne()");
}
