...
function education_page(	){
	$("#pagination").on("click", "li", function() {
		...
		if (aText == "上一页") {
			...
			education_getList(currentPage - 1, size);
			checkA();
			...
			$(".userListTable tbody").empty();
			education_getList($(this).text(), size);
			checkA();
		}
	})
	//分页栏右侧下拉列表的实现
...
//与后台获取数据，加载到页面上
function education_getList(pageNum, size) {
	currentPage = pageNum;
	//alert(termination);
				//alert(currentPage);
				...
				success : function(data) {
					...
					if (data.code == "200") {
						...
						for (var i = 0; i < length; i++) {
							var eduId = data.data.dataList[i].eduId;
							...
							var institution = data.data.dataList[i].institution;
							//var location = data.data.dataList[i].location;
							var certificate = data.data.dataList[i].certificate;
							var major = data.data.dataList[i].major;
							id='mytr'+eduId;
							...
							$(".userListTable tbody")
								.append('<tr id="mytr">'
												...
												+ major
												+ '</td>'
+ '<td><button type="button" class="btn btn-success btn-xs" onclick="education_updatePrepare();education_findOne('+eduId+')" data-toggle="modal" data-target="#editModal">修改</button><button type="button" class="btn btn-danger btn-xs" onclick="deleteOne('+eduId+')">删除</button></td>'
										+ '</tr>')
...
function education_findOne(id){
  ...
  //alert(result.children("td").get(1).innerHTML);  
	...
	$("#institution").val(cells[3].innerHTML);
	//$("#location").val(cells[4].innerHTML);
	$("#certificate").val(cells[4].innerHTML);
	$("#major").val(cells[5].innerHTML);
	var inp=document.getElementById("editModal"); 
	inp.setAttribute("eduId",id);//
...
function education_updateOne(){
	...
	row.eduId=inp2.attributes["eduId"].nodeValue;
	...
	row.institution=$("#institution").val();
	//row.location=$("#location").val();
	row.certificate=$("#certificate").val();
	row.major=$("#major").val();
	$.ajax({
		url : ip+'/demo/table/modify/education',
...
//修改结束
var hasId=true;
function education_updatePrepare(){
	if(hasId){
		$("#specialDiv").attr("style","display: none;");
		hasId=false;
	}
	$("#saveBtn").attr("onclick","");
	$("#saveBtn").attr("onclick","education_updateOne()");
}
